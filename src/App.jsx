...
import { Vote, Hammer, BookOpen, Users, ShieldCheck, ArrowLeft } from 'lucide-react';
...
export default function App() {
  ...
  const [pollResults, setPollResults] = useState({ 'Online Forum': 0, 'WhatsApp Group': 0, 'Email Updates': 0, 'Face-to-Face': 0 });
  const [voted, setVoted] = useState(localStorage.getItem('ktfcsa_voted') === 'true');
  ...
  useEffect(() => {
    return onSnapshot(doc(db, "polls", "communication"), (doc) => {
      if (doc.exists()) setPollResults(doc.data());
    });
  }, []);
  ...
  const handleVote = async (opt) => {
    ...
    const pollRef = doc(db, "polls", "communication");
    await updateDoc(pollRef, { [opt]: increment(1) });
    
    setVoted(true);
    localStorage.setItem('ktfcsa_voted', 'true');
    setTimeout(() => setActiveTab('blueprint'), 1200);
  };
    ...
    <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-[#d4af37] selection:text-black flex flex-col antialiased">
      ...
      <main className="max-w-6xl mx-auto p-4 md:p-8 flex-grow w-full">
        <nav className="flex bg-[#141414]/80 backdrop-blur-md border border-white/10 rounded-2xl mb-12 overflow-hidden shadow-2xl">
          {['polls', 'blueprint', 'join'].map((tab, i) => (
            <button 
              key={tab} 
              onClick={() => setActiveTab(tab)} 
              className={`flex-1 py-6 flex items-center justify-center gap-3 transition-all ${activeTab === tab ? 'bg-[#910b0b]/20 text-[#d4af37] border-b-2 border-[#d4af37]' : 'text-zinc-500 hover:text-zinc-300'}`}
            >
              {i === 0 && <Vote size={18} />}
              {i === 1 && <BookOpen size={18} />}
              {i === 2 && <Users size={18} />}
              <span className="font-black tracking-widest text-xs uppercase">{i+1}. {tab === 'blueprint' ? 'Fan Views' : tab === 'polls' ? 'The Setup' : 'Get Involved'}</span>
            </button>
          ))}
        </nav>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div className="lg:col-span-8 space-y-8">
            {activeTab === 'polls' && (
              <div className="bg-[#141414] border border-white/10 rounded-3xl p-8 shadow-xl animate-in fade-in slide-in-from-bottom-4 duration-500">
                <h2 className="text-2xl font-black uppercase text-[#d4af37] mb-2">Communication</h2>
                  ...
                  {Object.entries(pollResults).map(([opt, count]) => {
                    ...
                    return (
                      <button 
                        ...
                        className="group w-full text-left p-6 rounded-2xl border border-white/5 bg-white/[0.02] hover:border-[#d4af37]/50 transition-all"
                      >
            ...
            {activeTab === 'blueprint' && (
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="bg-[#141414] p-8 rounded-3xl border border-white/10 shadow-xl">
                  <h2 className="text-2xl font-black uppercase text-[#d4af37] mb-6">The Fan Wall</h2>
                  <form onSubmit={handlePostIdea} className="space-y-4">
                    <textarea 
                      value={feedback} 
                      onChange={(e) => setFeedback(e.target.value)} 
                      className="w-full bg-black/40 p-6 rounded-2xl border border-white/10 text-white min-h-[150px] focus:border-[#d4af37] outline-none placeholder:text-zinc-700 font-medium" 
                      placeholder="What should be our first priority as an association?" 
                    />
                ...
                {ideas.map(idea => (
                  <div key={idea.id} className="bg-[#141414]/60 p-8 rounded-3xl border border-white/5 italic text-zinc-100 text-lg font-medium">
                    "{idea.text}"
                  </div>
                ))}
              </div>
            ...
            {activeTab === 'join' && (
              <div className="bg-[#141414] p-16 rounded-3xl border border-white/10 text-center shadow-2xl animate-in zoom-in-95 duration-500">
                <Users size={40} className="mx-auto text-[#d4af37] mb-8" />
                <h2 className="text-4xl font-black uppercase mb-4 tracking-tighter">Join the Movement</h2>
                <p className="text-zinc-400 mb-10 max-w-md mx-auto text-lg leading-relaxed font-medium">We are currently forming the working group. If you have skills in legal, planning, or community organising, get in touch.</p>
                <button 
                  onClick={() => window.location.href='mailto:hello@ktfcsa.com'}
                  className="bg-[#d4af37] text-black px-12 py-5 rounded-2xl font-black uppercase tracking-widest text-sm hover:bg-white transition-all shadow-lg"
                >
          ...
          <div className="lg:col-span-4">
            <div className="bg-[#141414] border border-white/10 rounded-3xl p-8 shadow-xl sticky top-8">
              ...
              <ul className="space-y-6 text-sm font-semibold text-zinc-400">
        ...
        <div className="max-w-6xl mx-auto px-8 py-12 flex flex-col md:flex-row items-center justify-between gap-8">
          ...
          <div className="flex flex-col items-center md:items-end gap-4">
            <a 
              ...
              className="flex items-center gap-2 text-[#d4af37] text-[11px] font-black uppercase tracking-[0.2em] hover:text-white group"
            >
