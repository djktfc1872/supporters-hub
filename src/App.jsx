    ...
    <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-[#d4af37] selection:text-black flex flex-col antialiased">
      <div className="relative w-full overflow-hidden border-b-4 border-[#910b0b] bg-[#0a0a0a]">
        ...
        <div className="relative max-w-6xl mx-auto p-8 md:p-16 pt-24 flex flex-col md:flex-row items-center gap-10">
          <img src="/ktfcsa-logo.png" className="w-28 h-28 md:w-40 md:h-40 rounded-full border-4 border-[#d4af37] bg-black shadow-2xl object-contain" alt="KTFCSA Logo" />
          ...
          <div className="lg:col-span-8 space-y-8">
              ...
              <div className="bg-[#141414] border border-white/10 rounded-3xl p-8 shadow-xl animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {Object.entries(pollResults).map(([opt, count]) => {
                    ...
                    return (
                      <button key={opt} onClick={() => handleVote(opt)} className="group w-full text-left p-6 rounded-2xl border border-white/5 bg-white/[0.02] hover:border-[#d4af37]/50 transition-all">
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="bg-[#141414] p-8 rounded-3xl border border-white/10 shadow-xl">
                  ...
                  <form onSubmit={handlePostIdea} className="space-y-4">
                    <textarea value={feedback} onChange={(e) => setFeedback(e.target.value)} className="w-full bg-black/40 p-6 rounded-2xl border border-white/10 text-white min-h-[150px] focus:border-[#d4af37] outline-none placeholder:text-zinc-700 font-medium tracking-tight" placeholder="What should be our first priority as an association?" />
            ...
            {activeTab === 'join' && (
              <div className="bg-[#141414] p-16 rounded-3xl border border-white/10 text-center shadow-2xl animate-in zoom-in-95 duration-500">
                ...
                <button onClick={() => window.location.href='mailto:hello@ktfcsa.com'} className="bg-[#d4af37] text-black px-12 py-5 rounded-2xl font-black uppercase tracking-widest text-sm hover:bg-white transition-all shadow-lg shadow-[#d4af37]/10">
      ...
      <footer className="w-full bg-[#0a0a0a] border-t border-white/10 mt-20">
        <div className="max-w-6xl mx-auto px-8 py-12 flex flex-col md:flex-row items-center justify-between gap-8">
          <p className="text-zinc-500 text-[11px] uppercase font-black tracking-[0.3em]">Kettering Town Supporters' Association</p>
          <div className="flex flex-col items-center md:items-end gap-4">
            <a href="https://ktfcsa.com" className="flex items-center gap-2 text-[#d4af37] text-[11px] font-black uppercase tracking-[0.2em] hover:text-white group">
              <ArrowLeft size={14} className="group-hover:-translate-x-1 transition-transform" /> Back to Main Website
            </a>
            <p className="text-zinc-800 text-[10px] uppercase font-black tracking-[0.2em]">&copy; 2026 KTFCSA | Established by the Fans</p>
          </div>
        </div>
      </footer>
    </div>
